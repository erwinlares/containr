...
`containr` is also the second step in a three-package family for reproducible research workflows, from local project setup to containerization and high-throughput computing submission:
...
`containr` assumes your project already has enough structure to describe its R package environment.
...
The core workflow has four steps: generate a `Dockerfile`, build the image, inspect local images, and push the image to a registry.

The first three steps happen inside R. Authentication with the container registry happens once in the terminal before you push.
...
`build_image()` passes your `Dockerfile` to Podman or Docker and builds the image locally.
...
`push_image()` tags a local image with a registry path and pushes it to a container registry.

Before pushing, authenticate with the registry once in a terminal. `containr` checks whether you are logged in before attempting the push and errors with instructions if not. The UW-Madison authentication guide, including how to create a Personal Access Token with the right scopes, is here:
...
Use explicit version tags, such as `"1.0.0"`, for reproducibility. Avoid relying on `"latest"` for analyses that need to be reviewed, rerun, cited, or submitted to a cluster. The `"latest"` tag is overwritten on every push, which makes it harder to reconstruct which image was used for a specific result.
...
The `submitr` package handles the next step in the CHTC-oriented workflow: generating the HTCondor submit file, generating the executable script, uploading files to the submit node, submitting the job, monitoring progress, and retrieving results.
...
`renv` and containers solve related but different reproducibility problems.

`renv` records the R package versions used by your project. That is the right starting point because it makes the R package environment explicit.

A container goes one level lower. It captures the operating-system-level environment needed to install and run those packages: system libraries, command-line tools, and the base image that supplies the runtime environment.

`containr` connects these layers by using `renv.lock` as the source for the generated `Dockerfile`. In other words, the lockfile records the R package environment, and the container recipe turns that record into a portable runtime.

---

## Testing

`containr` wraps system commands (`podman`, `docker`) that cannot be tested end-to-end without a running container daemon, real images, and, for `push_image()`, a live registry. The test suite addresses this with a three-layer strategy.

**Layer 1 — Argument validation.** Pure R checks that bad arguments error correctly and required arguments are enforced. These tests run on every platform, including CRAN.

**Layer 2 — Command construction.** Tests that the correct system command is assembled from the supplied arguments. `dry_run = TRUE` causes the function to print the command without running it. Mocked bindings intercept internal container checks so these tests can run without Podman or Docker installed.

**Layer 3 — Integration.** Tests that call real system commands against a live container environment. These tests are guarded behind an explicit opt-in:
...
Integration tests never run automatically on CRAN or CI. They are intended for local pre-release verification.
...
- [toolero](https://github.com/erwinlares/toolero) — organize and scaffold research projects
