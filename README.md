...
**1. Generate a Dockerfile** — `generate_dockerfile()` reads your
`renv.lock` file, auto-detects the system libraries your packages need,
and writes a ready-to-use `Dockerfile`. Run this as many times as needed
while refining the configuration. It only writes a text file, so
...
directory. If you are starting a new project, `toolero::init_project()`
scaffolds a standard folder structure and initializes `renv` for you.
Once your project is set up, `renv::snapshot()` produces the lockfile
that `generate_dockerfile()` reads.
