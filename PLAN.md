...
in sequence. Intended for researchers who want the full build-and-push
workflow in one call after the first time setup.
...
`registry.doit.wisc.edu`. Will require updating
[`push_image()`](https://erwinlares.github.io/containr/reference/push_image.md)
to accept different authentication patterns.

### Singularity / Apptainer support

HPC environments (including some CHTC configurations) use Singularity or
Apptainer rather than Podman/Docker. Adding these as valid `tool` values
in
[`.resolve_tool()`](https://erwinlares.github.io/containr/reference/dot-resolve_tool.md)
is the entry point. Scoped for a future release alongside `submitr`’s
HPC support.

### Fix `.resolve_tool()` fallthrough logic

[`.resolve_tool()`](https://erwinlares.github.io/containr/reference/dot-resolve_tool.md)
prefers Podman but does not fall through to Docker when Podman is
installed but unresponsive (daemon not running). Currently,
[`.check_tool_responsive()`](https://erwinlares.github.io/containr/reference/dot-check_tool_responsive.md)
errors immediately instead of trying the next available tool. The fix:
[`.resolve_tool()`](https://erwinlares.github.io/containr/reference/dot-resolve_tool.md)
should check responsiveness as part of its selection logic and fall
through to Docker if Podman is unresponsive, rather than errorring out.
...
1.  Should `containerize()` accept an `image_id` or always use the most
    recently built image? The latter is more convenient but less
    explicit.

2.  Should
    [`push_image()`](https://erwinlares.github.io/containr/reference/push_image.md)
    eventually support `ghcr.io` and `quay.io` via a `registry`
    argument, or should separate registry-specific functions be added?

3.  When Singularity/Apptainer support arrives, should
