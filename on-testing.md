...
Layer 3 test adds two more guards on top of these:
`CONTAINR_TEST_NAMESPACE` and `CONTAINR_TEST_PROJECT`. Unlike
[`build_image()`](https://erwinlares.github.io/containr/reference/build_image.md),
which only needs a local daemon,
[`push_image()`](https://erwinlares.github.io/containr/reference/push_image.md)
needs somewhere real to push a throwaway test image to – and hardcoding
a destination (even a placeholder like the
`erwin.lares`/`container-registry` example used elsewhere in the docs)
would risk the test silently pushing to a project it was never actually
told to use. Requiring both variables be set explicitly means the test
only runs against a destination the developer chose on purpose, and
skips with a clear message otherwise. environment variable is set on a
machine without a container tool.
